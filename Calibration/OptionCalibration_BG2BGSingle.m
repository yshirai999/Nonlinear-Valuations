...
%                    (A-B)./((A+B)/2) ));
    f = @(x)(norm( [BGC( abs(x(1))+abs(x(4)),...
                            abs(x(2)),...
                            abs(x(5)),...
                            abs(x(2)),...
                            T,r(d),alpha_C,K_0,K_C,lambda,N,eta);...
                    BGC( abs(x(5))*abs(x(1))/(abs(x(5))+abs(x(3))),...
                            abs(x(2)),...
                            (abs(x(1))+abs(x(4)))*(abs(x(5))+abs(x(3)))/abs(x(1)),...
                            abs(x(2)),...
                            T,r(d),alpha_C,K_0,K_C,lambda,N,eta);
                    BGP( abs(x(5))*abs(x(1))/(abs(x(5))+abs(x(3))),...
                            abs(x(2)),...
                            (abs(x(1))+abs(x(4)))*(abs(x(5))+abs(x(3)))/abs(x(1)),...
                            abs(x(2)),...
                            T,r(d),alpha_P,K_0,K_P,lambda,N,eta);...
                    BGP( abs(x(1))+abs(x(4)),...
                            abs(x(2)),...
                            abs(x(5)),...
                            abs(x(2)),...
    ...
    param0 = [0.025505219338406,7.866812312323297,...
                0.068208498064262-0.000086633500582,...
                0.025505219338406+0.000076129920085,0.068208498064262];
    param0 = [2.549541e-02,7.869949e+00,6.828158e-02,2.557007e-02,6.819376e-02];
    %param0 = [2.370937e-02,7.869952e+00,6.349825e-02,2.557005e-02,6.819375e-02];
    bp = param0(1); cp = param0(2); Delta_bn = param0(3)-param0(5); Delta_bpU = param0(4)-bp; bnU = param0(5);
    
    [x,~,exitflagD] = fminsearch(f,[bp,cp,Delta_bn,Delta_bpU,bnU],optionsD);
    %[x,~,exitflagBG] = fminunc(f,[bp,cp,Delta_bn,Delta_bpU,bnU],optionsBG2BG);
    bp = abs(x(1)); cp = abs(x(2)); bn = abs(x(5))+abs(x(3));
    bpU = bp+abs(x(4)); bnU = abs(x(5));
    cn = cp; bpL = bp*bnU/bn; bnL = bn*bpU/bp;
...
grid on
box on
scatter([K_P;K_C]/K_0-1,max([P_Mod_U;C_Mod_U]-[P_Mod_L;C_Mod_L],0)./[(P_Mod_U+P_Mod_L)/2;(C_Mod_U+C_Mod_L)/2],'*')
