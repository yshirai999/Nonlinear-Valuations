...
options = optimset('MaxFunEval',500,'MaxIter',150,'Display','off','TolFun',1e-10,'TolX',1e-10);
...
try
    SPYMD = load(fullfile(varPath,strcat('SPYMD',num2str(SY),'DM',num2str(Delta),num2str(enforcegamma),num2str(enforceb),num2str(N)))); SPYMD = SPYMD.SPYMD;
    RC = load(fullfile(varPath,strcat('RC',num2str(SY),'DM',num2str(Delta),num2str(enforcegamma),num2str(enforceb),num2str(N)))); RC = RC.RC;
    mu_mod = load(fullfile(varPath,strcat('mu_modDM',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb),num2str(N)))); mu_mod = mu_mod.mu_mod;
    mu_mkt = load(fullfile(varPath,strcat('mu_mkt',num2str(SY),num2str(Delta),num2str(N)))); mu_mkt = mu_mkt.mu_mkt;
    err = load(fullfile(varPath,strcat('errDM',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb),num2str(N)))); err = err.err;
    ...
    for i = imax-2%imin+Delta:Delta:imax
        tic
        ...
        obj = @(theta) objU(u,abs([theta,bp,cp,bn,cn]),NN,enforcegamma,enforceb,lb,yp,yn)+objL(l,abs([theta,bp,cp,bn,cn]),NN,enforcegamma,enforceb,lb,yp,yn);
...
% b =  SPYMD(end,3);
% 
% xx = 0.00001:0.00001:1;
% Gp = a*(1-exp(-c*xx)).^(1/(1+gamma));
% Gm = (b/c)*(1-exp(-c*xx));
% figure
% hold on
% box on
...
% plot(xx,Gp)
% plot(xx,Gm)
% legend('$\Gamma_+$','$\Gamma_-$','interpreter','latex')
...
for i=1:length(quant)
    fprintf('%3.4f & %3.4f & %3.4d & %3.4f & %3.4f \\\\ \n', MDQ(i,[1,2,4,3,5]))
end

%% ADF test on nonstationarity of Delta Drivers
RCDelta = RC(:,1)-RC(:,2);

fprintf('Augmented Dickey-Fueller Test Rejects Delta DM Drivers Stationarity: %d\n', adftest(RCDelta))
...
box on
grid on
plot(d(3:end),RC(3:end,1),'-')
plot(d(3:end),RC(3:end,2),'--')
legend('Upper','Lower','interpreter','latex')
set(gca,'TickLabelInterpreter','latex')
str=strcat('SPY_RCvsMktReturn_DigitalMoments',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb));
fname=str;
...
box on
grid on
plot(d(3:end),RC(3:end,1)-RC(3:end,2),'-')
title('Difference between upper and lower DM drivers','interpreter','latex')
%legend('Upper-Lower','interpreter','latex')
set(gca,'TickLabelInterpreter','latex')
str=strcat('SPY_RCvsMktReturn_DigitalMoments',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb));
fname=str;
...
box on
grid on
plot(d(505:end),RC(505:end,1)-RC(505:end,2),'-')
title('Difference between upper and lower DM drivers in 2020','interpreter','latex')
set(gca,'TickLabelInterpreter','latex')
%set(gca, 'LooseInset', get(gca,'TightInset'))
xtickformat('MMM-yyyy')
str=strcat('SPY_RCvsMktReturn_DigitalMoments2020',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb));
fname=str;
saveas(gcf, fullfile(vizPath, fname), 'epsc');
hold off
...
box on
grid on
plot(datetime(Y(imin:end,1),'ConvertFrom','yyyymmdd'),Y(imin:end,2))
legend('SPY','interpreter','latex')
set(gca,'TickLabelInterpreter','latex')
str=strcat('SPY');
fname=str;
...
xx = 0.00001:0.00001:1;
%c = SPYMD(end,1); g = SPYMD(end,2); b = SPYMD(end,3); a = SPYMD(end,4);
...
% set(gca,'TickLabelInterpreter','latex')
% str=strcat('MeasureDistortionsDMm',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb));
% fname=str;
% saveas(gcf, fullfile(vizPath, fname), 'epsc');
% hold off
% 
...
% legend('$\Gamma_+$','interpreter','latex')
% set(gca,'TickLabelInterpreter','latex')
% str=strcat('MeasureDistortionsDMp',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb));
% fname=str;
% saveas(gcf, fullfile(vizPath, fname), 'epsc');
% hold off
...
set(gca,'TickLabelInterpreter','latex','yscale','log')
str=strcat('MeasureDistortionsDM',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb));
fname=str;
saveas(gcf, fullfile(vizPath, fname), 'epsc');
hold off
...
legend('Upper','Lower','interpreter','latex')
str=strcat('CorrDM',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb));
fname=str;
saveas(gcf, fullfile(vizPath, fname), 'epsc');
hold off
...
title('Difference between upper and lower DM correlations','interpreter','latex')
str=strcat('CorrDM',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb));
fname=str;
saveas(gcf, fullfile(vizPath, fname), 'epsc');
hold off
...
if strcmp(s,'Y')
    ...
    s = input(prompt, 's');
    if strcmp(s,'Y')
        save(fullfile(varPath,strcat('SPYMD',num2str(SY),'DM',num2str(Delta),num2str(enforcegamma),num2str(enforceb),num2str(N))),'SPYMD');
        save(fullfile(varPath,strcat('RC',num2str(SY),'DM',num2str(Delta),num2str(enforcegamma),num2str(enforceb),num2str(N))),'RC');
        save(fullfile(varPath,strcat('mu_modDM',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb),num2str(N))),'mu_mod');
        %save(fullfile(varPath,strcat('mu_mktDM',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb),num2str(N))),'mu_mkt');
        save(fullfile(varPath,strcat('ErrDM',num2str(SY),num2str(Delta),num2str(enforcegamma),num2str(enforceb),num2str(N))),'err');
    end
end
...
function T = objU(u,theta,N,enforcegamma,enforceb,lb,yp,yn)
...
function T = objL(l,theta,N,enforcegamma,enforceb,lb,yp,yn)
